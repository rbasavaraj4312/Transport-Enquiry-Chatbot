...
const Home = ({ user, setBusId, setUser }) => {
  ...
  const [fromStopFilter, setFromStopFilter] = useState(""); 
  const [toStopFilter, setToStopFilter] = useState(""); 
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 bg-gray-50 min-h-screen">
        ...
        {buses.map((bus) => {
          ...
          return (
            <Link
              ...
              <div
                key={bus._id}
                className="bg-white p-4 md:p-6 rounded-lg shadow-md border border-gray-200 flex items-center justify-between min-h-[100px] w-full m-5">
      ...
      <button
        ...
        className="fixed bottom-10 right-10 flex items-center justify-center h-18 w-18 rounded-full bg-blue-600 text-white shadow-lg hover:bg-blue-700 transition-colors duration-200 text-3xl font-bold">
        <Bot className="h-8 w-8" />
      </button>

      {showChatBot && <ChatBot setShowChatBot={setShowChatBot} user={user} />}
    </div>
  );
};
